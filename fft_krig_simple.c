...
int main (int argc, char *argv[]) {
    ...
    int i,j,k,size_fin,Nchan, nbins, **fg_num=NULL, band=0,Nchanall,Nfreq,mm,Nst=0,Nchan_actual=192;
...
if (argc < 2){
    fprintf(stderr,"Usage: <options> %s input_extension Nchan nbins pol maxu output_extension Nchan_output Starting_channel_number \n",argv[0]);
...
// Check to see that this will output something that works
    
    if (Nchan_actual+Nst > Nchan){
        printf("Too many channels given starting channel number, exiting\n");
        return 1;
    }
