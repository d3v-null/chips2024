...
int main (int argc, char *argv[]) {
  ...
  char syslogfile[1024],infilename[1024],maskfile[1024],filename_real[1024],filename_real2[1024],filename_flags[1024],debugfile[1024],filename_real3[1024],filename_uvw[1024],filename_noise[1024],infilestub[1024],*ext=NULL,*polval=NULL;
  char *obsid=NULL;
...
if (argc < 2){
    fprintf(stderr,"Usage: %s <options> obsid band Nchan subdirectory/uvfits_name_stub pol \n",argv[0]);
  ...
  //      printf("Here1\n");
		...
		for (k=0;k<Neta;k++){  //loop over eta modes
			...
			//		printf("1 %d %f %f\n",k,distance,factor_kpa*BW/DM*distance);
				...
				if (((float) k < 4)&&(distance > 0)&&(distance < 100)){
			//		printf("2 %d %f %f\n",k,distance,factor_kpa*BW/DM*distance);
