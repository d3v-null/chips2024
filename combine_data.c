...
while(fscanf(fin,"%s",fileext) != EOF){
    ...
    /* Compute weights contribution */
        ...
        if ((fptrv = fopen(weightsfilename,"r")) == NULL){
            ...
            if (fread(w1,sizeof(float),size_fin*Nchan,fptrv) != size_fin*Nchan) fprintf(flog,"Error: input file is the incorrect size. File: %s\n",weightsfilename);
            ...
            for (i=0;i<size_half*Nchan;i++) weights[i] += w1[i];///norm[ch];
...
noised = calloc(size_half*Nchan,sizeof(float complex));
				...
				if ((fptrv = fopen(vistfilename,"r")) == NULL){
					...
					if (fread(vist,sizeof(float complex),size_fin*Nchan,fptrv) != size_fin*Nchan) fprintf(flog,"Error: input file is the incorrect size. File: %s\n",vistfilename);
...
/***********************/
				...
				if ((fptrv = fopen(visdfilename,"r")) == NULL){
					...
					if (fread(visd,sizeof(float complex),size_fin*Nchan,fptrv) != size_fin*Nchan) fprintf(flog,"Error: input file is the incorrect size. File: %s\n",visdfilename);
...
/***********************/
				...
				if ((fptrv = fopen(noisetfilename,"r")) == NULL){
					...
					if (fread(noiset,sizeof(float complex),size_fin*Nchan,fptrv) != size_fin*Nchan) fprintf(flog,"Error: input file is the incorrect size. File: %s\n",noisetfilename);
...
/***********************/
				...
				if ((fptrv = fopen(noisedfilename,"r")) == NULL){
					...
					if (fread(noised,sizeof(float complex),size_fin*Nchan,fptrv) != size_fin*Nchan) fprintf(flog,"Error: input file is the incorrect size. File: %s\n",noisedfilename);
